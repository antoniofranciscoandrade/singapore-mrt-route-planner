...
const addInterchangeEdges = (interchanges, graph = Graph()) => {
  ...
  return addAllEdges(pairsOfStations, graph, 10000);
};
...
export const getDirections = (route, lines) => {
  ...
  Object.values(lines).forEach((line) => {
    allLines.push(line);
    if (line[0].name === line[line.length - 1].name)
      allLines.push([...line].reverse());
  });
  ...
  for (let curr = 0; curr < route.length - 1; curr++) {
    ...
    for (const line of allLines) {
      const foundIdx = line.findIndex(
        (station) => station.name === routeStation
      );
      if (foundIdx === -1) continue;
      const linePrevStation = foundIdx === 0 ? '' : line[foundIdx - 1].name;
      ...
      if (foundIdx === line.length - 1) {
        ...
        direction =
          nextStation === line[foundIdx + 1].name
